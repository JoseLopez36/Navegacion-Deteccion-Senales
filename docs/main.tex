...
lo utiliza para las señales de 30 $km/h$ y 60 $km/h$.\\
...
unas imágenes (alrededor de 70 imágenes), se utiliza un script Python para aplicar transformaciones en las imágenes, como rotaciones o cambios de luz. Al
...
Por fin, el dataset de test se compone unicamente de imágenes que provienen de la simulación, con 22 imágenes para cada tipo de señal.\\
...
En la Figura~\ref{fig:entrenamiento} se muestran los resultados de un entrenamiento del modelo con 23 épocas. Se observa que las pérdidas y la eficiencia 
se estabilizan al final del entrenamiento. Sin embargo, durante el entrenamiento, las pérdidas y la eficiencia varían de manera significativa. Estas 
variaciones pueden explicarse por las capas de Dropout. En efecto, en nuestro modelo, estas capas desactivan las neuronas con una probabilidad de 0.3 para 
luchar contra el overfitting. Esto obliga el modelo a encontrar diferentes maneras reconocer las características de la imagen. \\

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{media_CNN/final_result.png}
    \caption{Resultados de un entrenamiento con la evolución de las pérdidas a la izquierda, y de la eficiencia a la derecha}
    \label{fig:entrenamiento}
\end{figure}

\newpage
Varios otros entrenamientos han sido efectuados con varios valores de épocas. Si el número de épocas supera 25, un overfitting aparece y los resultados
sobre la clase de señales de 60 $km/h$ son muy bajos. Al otro lado con menos épocas, las pérdidas y la eficiencia varían demasiado como se observa en la
Figura~\ref{fig:entrenamiento}. 

\subsubsection{Test sobre señales de la simulación}

Antes de comprobar este modelo en la simulación, se lo comproba con un dataset de Test con señales que provienen de la simulación. Este dataset se compone de 
...
\begin{figure}[H]
    ...
    \caption{Matriz de confusión del test sobre el test del modelo}
    \label{fig:CM}
\end{figure}

En esta matriz de confusión, se puede ver que el modelo adivina bien las señales de 30 $km/h$ y de 90 $km/h$, con porcentajes de aciertos igual a un
86 \% y un 81 \% respectivamente. Estos porcentajes son suficientes, porque en la simulación no se tratan de imágenes aisladas, sino de un flujo video. Entonces se 
utiliza un filtro para suavizar las predicciones. 

En cuanto a la clasificación de las señales de 60 $km/h$, los resultados son más bajos, con un porcentaje de aciertos igual a un 54 \%. Se observa que
el modelo confunde las señales de 60 $km/h$ con las señales de 90 $km/h$. En efecto, un 6 y un 9 son muy parecidos, lo que puede complicar la clasificación
de estas señales. 
